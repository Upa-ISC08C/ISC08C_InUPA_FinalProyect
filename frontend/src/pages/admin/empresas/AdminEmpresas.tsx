...
export function AdminEmpresas() {
  ...
  const [estado, setEstado] = useState<"todos" | "activas" | "inactivas">(
    "todos",
  );
  const [filtroIndustria, setFiltroIndustria] = useState("todas");
  const [vista, setVista] = useState<"cuadricula" | "lista">(
    () => (localStorage.getItem("inupa_empresas_view") as any) || "cuadricula",
  );
  ...
  useEffect(() => {
    localStorage.setItem("inupa_empresas_view", vista);
  }, [vista]);
  ...
  const filtradas = useMemo(() => {
    const list = empresas.filter((e) => {
      const matchQ = (e.nombre + (e.industria || "") + (e.ciudad || ""))
        .toLowerCase()
        .includes(q.toLowerCase());
      const matchEst =
        estado === "todos" || (estado === "activas" ? e.activa : !e.activa);
      const matchInd =
        filtroIndustria === "todas" || e.industria === filtroIndustria;
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            {empresas.length} registradas · {activas} activas
          </p>
        </div>
        <Button
          onClick={() => setEdit("new")}
            ...
            >
              <CardContent className="p-5">
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-3">
                    <div className="size-11 rounded-xl overflow-hidden flex-none flex items-center justify-center bg-muted">
                      {e.logo_url ? (
                        ...
                        <div className="size-full bg-[#003366] text-white flex items-center justify-center font-bold text-sm">
                  ...
                  <span
                    className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${e.activa ? "bg-[#DCFCE7] text-[#16A34A]" : "bg-[#FEE2E2] text-[#DC2626]"}`}
                  >
                ...
                {e.sitio_web && (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="inline-flex items-center gap-1 text-xs text-[#003366] font-semibold hover:underline mt-2"
                  >
                ...
                <div className="flex gap-1 mt-4 pt-3 border-t border-border">
                  ...
                  <button
                    ...
                    className={`flex items-center gap-1 text-xs font-semibold px-2.5 py-1.5 rounded-lg ${e.activa ? "text-[#E74C3C] hover:bg-[#FEE2E2]" : "text-[#16A34A] hover:bg-[#DCFCE7]"}`}
                  >
                    {e.activa ? (
                      <>
                  ...
                  <button
                    onClick={() =>
                      setConfirmDelete({ id: e.id, nombre: e.nombre })
                    }
                        ...
                        <div className="flex items-center gap-3">
                          <div className="size-9 rounded-lg overflow-hidden flex-none flex items-center justify-center bg-muted">
                            {e.logo_url ? (
                              ...
                              <div className="size-full bg-[#003366] text-white flex items-center justify-center text-xs font-bold">
                                {e.nombre.substring(0, 2).toUpperCase()}
                              </div>
                            )}
                          </div>
                      ...
                      <td className="px-5 py-3">
                        <div className="flex gap-1 justify-end">
                          ...
                          <button
                            ...
                            className={`size-8 flex items-center justify-center rounded-lg transition-colors ${e.activa ? "text-[#E74C3C] hover:bg-[#FEE2E2]" : "text-[#16A34A] hover:bg-[#DCFCE7]"}`}
          ...
          <DialogContent className="sm:max-w-md rounded-2xl border-0 shadow-2xl p-6 bg-background animate-in fade-in zoom-in-95 duration-200">
            ...
            >
              ...
              <span className="font-extrabold text-sm uppercase tracking-wider">
                {confirm.activa ? "¿Desactivar Empresa?" : "¿Activar Empresa?"}
              </span>
            </div>
              ...
              <p className="text-sm text-foreground leading-relaxed">
                {confirm.activa ? (
                  <>
                    ...
                    <strong>{confirm.nombre}</strong>? Esto ocultará su
                    información y afectará a sus vacantes publicadas.
                  </>
                ) : (
                  <>
                    ¿Deseas activar la empresa <strong>{confirm.nombre}</strong>{" "}
                    para que vuelva a estar visible en el sistema?
                  </>
                )}
              </p>
...
export function EmpresaDialog({
  ...
  const [otroIndustria, setOtroIndustria] = useState(
    item?.industria && !INDUSTRIAS_COMUNES.includes(item.industria)
      ? item.industria
      : "",
  );
  ...
  const subirLogo = async (file?: File) => {
    ...
    if (file.size > 50 * 1024 * 1024) {
      setError(
        "El logotipo supera el límite de tamaño permitido (máximo 50MB).",
      );
      return;
    }
    try {
  ...
  const guardar = async () => {
    const finalIndustria =
      selIndustria === "Otro" ? otroIndustria.trim() : selIndustria;
    ...
    try {
      ...
      const msg =
        e?.response?.data?.error || "Ha ocurrido un error al guardar.";
      setError(msg);
    ...
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="sm:max-w-3xl max-h-[90vh] overflow-y-auto rounded-2xl border-0 shadow-2xl p-6 bg-background animate-in fade-in zoom-in-95 duration-200">
        <div className="bg-[#001A33] text-white px-6 py-4 rounded-t-2xl flex items-center gap-2 -mx-6 -mt-6 border-b border-white/10 h-14">
          ...
          <span className="font-extrabold text-sm uppercase tracking-wider">
            {item ? "Editar Empresa" : "Nueva Empresa"}
          </span>
        </div>
        <div className="space-y-4 mt-5">
          ...
          <div className="grid md:grid-cols-4 gap-4 items-start">
                ...
                {form.logo_url ? (
                  <>
                    ...
                    <button
                      ...
                      className="absolute top-1.5 right-1.5 size-6 rounded-full bg-black/60 text-white flex items-center justify-center hover:bg-black/80 transition-colors shadow"
                    >
            ...
            <div className="md:col-span-3 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4 items-start">
                <F label="Industria *" error={fieldErrors.industria}>
                  <div className="space-y-2">
                    ...
                    {selIndustria === "Otro" && (
                      <Input
                        ...
                        className="rounded-xl h-10 border-border bg-muted/20 focus-visible:ring-[#003366] animate-in slide-in-from-top-1 duration-150"
                        placeholder="Escribe la industria..."
                      />
                    )}
                  </div>
                </F>
                <F label="Tamaño (empleados) *" error={fieldErrors.tamano}>
                  <Input
          ...
          <F label="Descripción *" error={fieldErrors.descripcion}>
            <Textarea
              rows={3}
              value={form.descripcion}
              onChange={(e) => set("descripcion", e.target.value)}
              className="rounded-xl border-border bg-muted/20 focus-visible:ring-[#003366] resize-none"
          ...
          <F label="Correo de contacto (Opcional)">
            <Input
          ...
          <F label="Dirección física (Opcional)">
            <Input
          ...
          <label className="flex items-center gap-2.5 text-sm text-[#2C3E50] cursor-pointer select-none font-medium mt-2">
            <input
              ...
              className="size-4 rounded border-border text-[#003366] focus:ring-[#003366] cursor-pointer"
            />
        ...
        <div className="flex justify-end gap-3 mt-6 pt-3 border-t border-border">
          <Button
            ...
            onClick={onClose}
            disabled={saving}
            className="rounded-xl h-10 hover:bg-muted font-semibold"
          >
            Cancelar
          </Button>
          <Button
            onClick={guardar}
            disabled={saving}
            className="bg-[#003366] hover:bg-[#002244] text-white rounded-xl h-10 px-5 font-semibold flex items-center gap-1.5 active:scale-95 transition-all"
          >
            {saving ? (
...
function F({ label, children, error }: any) {
  return (
    <div className="space-y-1.5">
      <Label
        className={`text-xs font-bold uppercase tracking-wider ${error ? "text-red-500" : "text-muted-foreground"}`}
      >
        {label}
      </Label>
      <div
