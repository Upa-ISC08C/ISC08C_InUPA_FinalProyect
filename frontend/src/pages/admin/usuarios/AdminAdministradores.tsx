...
import {
  ShieldAlert, Search, Loader2, ShieldCheck, Ban, CheckCircle2, KeyRound,
  LayoutGrid, List, Lock, Mail, AlertTriangle, User, Edit, Plus, Shield,
} from "lucide-react";
...
export function AdminAdministradores() {
  ...
  const abrirEdicion = (u: AdminUser) => {
    ...
    setFormData({
      ...
      matricula_o_rfc: u.matricula_o_rfc || "",
      password: "",
    });
    setErrorModal("");
    setModalOpen(true);
  ...
  const guardarAdmin = async (e: React.FormEvent) => {
    ...
    if (!formData.correo_institucional.trim()) {
      setErrorModal("El correo institucional es obligatorio.");
      return;
    }
    if (!editingAdmin && !formData.password.trim()) {
      setErrorModal("La contraseña es obligatoria para nuevos administradores.");
      return;
    }
        ...
        <Card className="border-0 shadow-sm"><CardContent className="p-0">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {filtrados.map((u) => {
                  const esMismoOProtegido = u.correo_institucional === "admin@upa.edu.mx";
                  return (
                    <tr key={u.id} className="border-b border-border last:border-0 hover:bg-muted/40 transition-colors">
                      ...
                      <td className="px-5 py-3">
                        <div className="flex items-center justify-end gap-1">
                          <button onClick={() => abrirEdicion(u)} title="Editar administrador" className="inline-flex items-center gap-1 text-xs font-semibold px-2.5 py-1.5 rounded-lg text-[#003366] hover:bg-[#003366]/[0.08] transition-colors"><Edit className="size-3.5" />Editar</button>
                          <button onClick={() => resetPassword(u)} title="Restablecer contraseña" className="inline-flex items-center gap-1 text-xs font-semibold px-2.5 py-1.5 rounded-lg text-[#003366] hover:bg-[#003366]/[0.08] transition-colors"><KeyRound className="size-3.5" />Reset</button>
                          {esMismoOProtegido ? (
                ...
                <CardContent className="p-5">
                  ...
                  <div className="flex items-center gap-1 mt-4 pt-3 border-t border-border">
                    ...
                    {esMismoOProtegido ? (
                      ...
                      <button onClick={() => toggleActivo(u)} className={`inline-flex items-center gap-1 text-xs font-semibold px-2.5 py-1.5 rounded-lg ml-auto transition-colors ${u.activo ? "text-[#E74C3C] hover:bg-[#FEE2E2]" : "text-[#16A34A] hover:bg-[#DCFCE7]"}`}>
                        {u.activo ? <><Ban className="size-3.5" />Suspender</> : <><CheckCircle2 className="size-3.5" />Activar</>}
                      </button>
                    )}
                  </div>
          ...
          <DialogContent className="sm:max-w-xl rounded-2xl border-0 shadow-2xl p-0 bg-background overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="bg-[#001A33] text-white px-6 py-4 flex items-center gap-2 border-b border-white/10 h-14">
              ...
              <span className="font-extrabold text-sm uppercase tracking-wider">
                {editingAdmin ? "Editar Administrador" : "Registrar Administrador"}
              </span>
            </div>

            <form onSubmit={guardarAdmin} className="p-6 space-y-4">
              {errorModal && (
                <div className="p-3 rounded-xl bg-red-50 border border-red-100 text-[#E74C3C] text-xs font-semibold flex items-center gap-2">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <Input
                    placeholder="Ej. PEJ901201"
                    value={formData.matricula_o_rfc}
                    onChange={(e) => setFormData({ ...formData, matricula_o_rfc: e.target.value })}
                    className="h-10 rounded-xl text-sm"
                  />
                </div>
                ...
                <div className="space-y-1.5 md:col-span-2">
                  ...
                  <Input
                    required
                    type="email"
                    placeholder="Ej. administrador@upa.edu.mx"
                ...
                <div className="space-y-1.5 md:col-span-2">
                  <label className="text-xs font-bold text-muted-foreground">
                    Contraseña {editingAdmin ? "(Dejar en blanco para mantener la actual)" : "*"}
                  </label>
                  <Input
                    type="password"
              ...
              <div className="flex justify-end gap-3 pt-3 border-t border-border">
                <Button type="button" variant="outline" onClick={() => setModalOpen(false)} className="rounded-xl h-10 px-4 active:scale-95 transition-all">
                  Cancelar
                </Button>
          ...
          <DialogContent className="sm:max-w-md rounded-2xl border-0 shadow-2xl p-6 bg-background animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="space-y-4 mt-4">
              <p className="text-sm text-foreground leading-relaxed">
                {confirm.activo ? (
                  <>¿Estás seguro de que deseas suspender al administrador <strong>{confirm.nombre_completo}</strong>? No podrá acceder al panel de administración mientras esté suspendido.</>
                ) : (
                  <>¿Deseas activar al administrador <strong>{confirm.nombre_completo}</strong> para permitirle acceder nuevamente al panel?</>
                )}
              </p>
              <div className="flex justify-end gap-3 pt-3 border-t border-border">
