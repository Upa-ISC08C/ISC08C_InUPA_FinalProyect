...
import {
  ...
  LayoutGrid, List, Eye, Lock, Mail, Phone, MapPin, Link2, GraduationCap,
} from "lucide-react";
...
export function AdminUsuarios() {
  ...
  const toggleActivo = async (u: AdminUser) => {
    setAviso("");
    try { await adminService.updateUser(u.id, { activo: !u.activo }); cargar(); }
    catch (e: any) { setAviso(e?.response?.data?.error || "No se pudo cambiar el estatus."); }
  };
  ...
  return (
    <div className="space-y-5 animate-in fade-in duration-300">
      <div>
        <h1 className="text-2xl font-bold text-foreground tracking-tight">Usuarios registrados</h1>
        <p className="text-sm text-muted-foreground mt-0.5">{usuarios.length} usuarios · {activos} activos · {usuarios.length - activos} suspendidos</p>
      </div>
      ...
      {verId && <PerfilDialog id={verId} onClose={() => setVerId(null)} onChanged={() => { cargar(); }} onAviso={setAviso} />}
    </div>
  );
}
...
// ---------------------------------------------------------------------------
function PerfilDialog({ id, onClose, onChanged, onAviso }: { id: string; onClose: () => void; onChanged: () => void; onAviso: (s: string) => void }) {
  const [u, setU] = useState<AdminUserDetail | null>(null);
  const [loading, setLoading] = useState(true);
  const [busy, setBusy] = useState(false);

  const cargar = () => { setLoading(true); adminService.getUser(id).then(setU).catch(() => setU(null)).finally(() => setLoading(false)); };
  useEffect(() => { cargar(); /* eslint-disable-next-line */ }, [id]);

  const accion = async (fn: () => Promise<any>, msg: string) => {
    setBusy(true);
    try { await fn(); onAviso(msg); cargar(); onChanged(); } catch (e: any) { onAviso(e?.response?.data?.error || "No se pudo completar la acción."); } finally { setBusy(false); }
  };

  const p = u?.perfil;
  const esAdmin = u?.rol === "admin";
  const Row = ({ icon: Icon, children }: any) => (<div className="flex items-center gap-2 text-sm text-foreground"><Icon className="size-4 text-muted-foreground shrink-0" />{children}</div>);

  return (
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="max-w-lg">
        <DialogHeader><DialogTitle>Perfil del usuario</DialogTitle></DialogHeader>
        {loading ? (
          <div className="flex justify-center py-12"><Loader2 className="size-6 animate-spin text-[#003366]" /></div>
        ) : !u ? (
          <p className="text-sm text-muted-foreground py-8 text-center">No se pudo cargar el perfil.</p>
        ) : (
          <div className="space-y-4">
            <div className="flex items-center gap-4">
              <div className="size-16 rounded-full bg-[#003366] flex items-center justify-center text-white text-lg font-bold overflow-hidden flex-none">
              ...
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  <p className="font-bold text-foreground text-lg leading-tight">{u.nombre_completo}</p>
                  <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${u.rol === "admin" ? "bg-[#003366]/[0.1] text-[#003366]" : "bg-muted text-muted-foreground"}`}>{u.rol}</span>
                </div>
                {p?.titular_profesional && <p className="text-sm text-muted-foreground truncate">{p.titular_profesional}</p>}
                <p className="text-xs text-muted-foreground">{u.matricula_o_rfc}</p>
              </div>
            </div>

            <div className="grid grid-cols-1 gap-2 rounded-xl border border-border p-3">
              <Row icon={Mail}>{u.correo_institucional} {u.email_verificado ? <span className="text-[10px] text-[#16A34A] font-semibold ml-1">✓ verificado</span> : <span className="text-[10px] text-[#F59E0B] font-semibold ml-1">sin verificar</span>}</Row>
              {u.carrera && <Row icon={GraduationCap}>{u.carrera}{u.cuatrimestre ? ` · ${u.cuatrimestre}º cuatrimestre` : ""}</Row>}
              {p?.telefono && <Row icon={Phone}>{p.telefono}</Row>}
              {p?.ubicacion && <Row icon={MapPin}>{p.ubicacion}</Row>}
              {p?.github_url && <Row icon={Link2}><a href={p.github_url} target="_blank" rel="noreferrer" className="text-[#003366] hover:underline truncate">{p.github_url}</a></Row>}
              {p?.linkedin_url && <Row icon={Link2}><a href={p.linkedin_url} target="_blank" rel="noreferrer" className="text-[#003366] hover:underline truncate">{p.linkedin_url}</a></Row>}
            </div>

            {p?.biografia && <div><p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-1">Acerca de</p><p className="text-sm text-foreground leading-relaxed">{p.biografia}</p></div>}

            <div className="flex flex-wrap gap-2 pt-2 border-t border-border">
              <Button variant="outline" size="sm" disabled={busy} onClick={() => accion(() => adminService.resetPassword(u.id), `Código de restablecimiento enviado a ${u.correo_institucional}`)}>
                <KeyRound className="size-3.5 mr-1" />Restablecer contraseña
              </Button>
              {esAdmin ? (
                <span className="inline-flex items-center gap-1 text-xs font-semibold px-3 py-2 rounded-lg text-muted-foreground"><Lock className="size-3.5" />Administrador protegido</span>
              ) : (
                <Button variant="outline" size="sm" disabled={busy}
                  onClick={() => accion(() => adminService.updateUser(u.id, { activo: !u.activo }), u.activo ? "Usuario suspendido" : "Usuario activado")}
                  className={u.activo ? "text-[#E74C3C]" : "text-[#16A34A]"}>
                  {u.activo ? <><Ban className="size-3.5 mr-1" />Suspender</> : <><CheckCircle2 className="size-3.5 mr-1" />Activar</>}
                </Button>
              )}
            </div>
          </div>
