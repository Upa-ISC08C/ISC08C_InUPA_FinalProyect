...
const stats = [
  ...
  { value: "180+", label: "Empresas aliadas" },
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      google.accounts.id.renderButton(googleBtnRef.current, {
        theme: "outline", size: "large", text: "continue_with", width: 360, locale: "es",
      });
    };
    if ((window as any).google?.accounts?.id) { render(); return; }
    const ex = document.getElementById("google-gsi-script");
    if (ex) { ex.addEventListener("load", render); return; }
    const s = document.createElement("script");
    s.src = "https://accounts.google.com/gsi/client";
    s.async = true; s.defer = true; s.id = "google-gsi-script"; s.onload = render;
    ...
    <div className="min-h-screen flex bg-white font-[Poppins,sans-serif]">
        ...
        <div className="relative z-10 flex flex-col h-full p-12 xl:p-16">
          ...
          <div className="grid grid-cols-3 gap-4 pt-6 border-t border-white/10">
            {stats.map(({ value, label }) => (
              <div key={label}>
                <p className="text-[#FFD700] text-xl font-bold">{value}</p>
      ...
      <div className="flex-1 flex items-center justify-center p-8 bg-white">
        <div className="w-full max-w-[400px] space-y-6">
          ...
          {/* Google OAuth (botón oficial de Google) */}
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <Input id="email" type="email" placeholder="nombre@alumnos.upa.edu.mx" value={email}
          ...
          <div className="flex items-center gap-2 p-3 rounded-xl bg-[#F5F7FA] border border-[#E5E7EB]">
            ...
            <p className="text-xs text-[#7F8C8D]">
              ...
              <span className="font-semibold text-[#2C3E50] select-all">admin@upa.edu.mx</span>{" "}
              (inicia sesión normal; entrarás al panel).
            </p>
          </div>
        </div>
      </div>
